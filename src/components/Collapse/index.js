...
const CollapseComponent = () => {
  ...
  const handleToggleAnswer = (questionIndex, index, answerIndex) => {
    const selectedQuestionKey = `${index}-${questionIndex}-${answerIndex}`;
    const isAnswerOpen = openAnswers.includes(selectedQuestionKey);

    setOpenAnswers((prevOpenAnswers) =>
      isAnswerOpen
        ? prevOpenAnswers.filter((key) => key !== selectedQuestionKey)
        : [...prevOpenAnswers, selectedQuestionKey]
    );
  };
                  ...
                  <div key={questionIndex}>
                    ...
                    {item.answers.map((answer, answerIndex) => (
                      <ContainerAnswert key={answerIndex}>
                        ...
                        <Collapse in={openAnswers.includes(`${index}-${questionIndex}-${answerIndex}`)}>
                          <Typography>
